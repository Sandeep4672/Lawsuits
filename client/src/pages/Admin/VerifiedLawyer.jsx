...
export default function AllLawyers() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8000/admin/lawyers",{
      ...
      setLawyers(res.data.verifiedLawyers); // assumed format: array of lawyers
    });
  }, []);
    ...
    <div className="pt-24 px-6 bg-gradient-to-br from-green-100 to-green-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {lawyers.map((lawyer) => (
          <div
            key={lawyer._id}
            onClick={() => navigate(`/admin/lawyer/${lawyer._id}`)}
            className="p-4 bg-green-50 rounded-lg shadow hover:shadow-md cursor-pointer"
          >
            <h3 className="text-lg font-semibold">{lawyer.fullLawyerName}</h3>
            <p className="text-gray-600">{lawyer.professionalEmail}</p>
