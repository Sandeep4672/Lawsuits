...
export default function CaseDetails() {
 ...
 const navigate = useNavigate();
    ...
    <div className="min-h-screen pt-40 mx-auto py-20 px-6 sm:px-10  bg-gradient-to-br from-green-100 to-green-50">
      ...
      <div className="space-y-3 text-base text-gray-700">
        ...
        {caseData.partiesInvolved?.length > 0 && (
          <p>
            <span className="font-semibold text-cyan-800">Parties Involved:</span>{" "}
        ...
        {caseData.dateOfJudgment && (
          <p>
            <span className="font-semibold text-cyan-800">Date of Judgment:</span>{" "}
  ...
  <div className="mt-8 flex justify-center">
    <a
      href={
        caseData.pdfUrl.endsWith(".pdf")
          ? caseData.pdfUrl
          : caseData.pdfUrl + ".pdf"
      }
      download={
        caseData.title
          ? `${caseData.title.replace(/\s+/g, "_")}.pdf`
          : "case_document.pdf"
      }
    ...
    >
      ...
      {caseData.summary ? (
        <div className="mt-10 bg-white p-6 rounded-xl shadow text-gray-700 leading-relaxed whitespace-pre-wrap">
          <h2 className="text-xl font-semibold mb-4 text-green-700">Case Summary</h2>
          <p>{caseData.description || caseData.summary}</p>
        </div>
      ) : (
        <p className="mt-6 text-gray-500 italic">No summary or description provided.</p>
      )}
    </div>
    </>
  );
